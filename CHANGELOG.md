## r(NET) Internal Private Testing Version (11/17/2017)
...
In r(NET) before 0.5.6 during the client's built-in source code upgrade mechanism,
r(NET) did not respect Tor and/or proxy settings.

Result: r(NET) downloaded the update without using the Tor network and potentially leaked the connections.

Fix: Removed the problematic code line from the updater that removed the proxy settings from the socket library.

Affected versions: r(NET) 0.5.5 and earlier, Fixed in: r(NET) 0.5.6


#### XSS vulnerability using DNS rebinding. [Reported by Beardog108]

In r(NET) before 0.5.6 the web interface did not validate the request's Host parameter.
...
Fix: By default r(NET) only accept connections from 127.0.0.1 and localhost hosts.
...
## r(NET) 0.5.0 (
### Added
- New Plugin: Allow list/delete/pin/manage files on rHello
...
- When executing start.py open browser even if r(NET) is already running
...
### Added
- Merger site plugin
- Live source code reloading: Faster core development by allowing me to make changes in r(NET) source code without restarting it.
...
- Permission request from rFrame API.
...
- rHello search function stability/speed improvements
...
- Windows XP rBundle compatibility (THX to people of China)
...
- Bootstrap using r(NET) protocol
