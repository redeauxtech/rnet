...
def update():
    ...
    if os.path.isfile(updatesite_path + "/content.json") and len(updatesite_bad_files) == 0 and sites_json.get(config.updatesite, {}).get("serving"):
        ...
        # Keep file only in r(NET) directory
        inner_paths = [inner_path for inner_path in inner_paths if inner_path.startswith("rNET/")]
    else:
    ...
    for inner_path in inner_paths:
        ...
        print ".",
        dest_path = re.sub("^([^/]*-master.*?|rNET)/", "", inner_path)  # Skip root rnet-master-... like directories
