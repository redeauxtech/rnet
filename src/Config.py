...
class Config(object):
    ...
    def createArguments(self):
        ...
        if this_file.endswith("/Contents/Resources/core/src/Config.py"):
            ...
            if this_file.startswith("/Application") or this_file.startswith("/private") or this_file.startswith(os.path.expanduser("~/Library")):
                ...
                start_dir = os.path.expanduser("~/Library/Application Support/rNET").decode(sys.getfilesystemencoding())
            else:
            ...
            config_file = start_dir + "/rnet.conf"
            ...
            data_dir = start_dir + "/data"
            log_dir = start_dir + "/log"
        elif this_file.endswith("usr/share/rnet/src/Config.py"):
            # Running from non-writeable location, e.g., AppImage
            start_dir = os.path.expanduser("~/rNET").decode(sys.getfilesystemencoding())
            config_file = start_dir + "/rnet.conf"
            data_dir = start_dir + "/data"
            log_dir = start_dir + "/log"
        else:
            config_file = "rnet.conf"
            data_dir = "data"
            log_dir = "log"
        ...
        self.parser.add_argument('--version', action='version', version='rNET %s r%s' % (self.version, self.rev))
    ...
    def parse(self, silent=False, parse_config=True):
        ...
        if silent:  # Restore original functions
            if self.parser.exited and self.action == "main":  # Argument parsing halted, don't start r(NET) with main action
                self.action = None
